...
from auth import AuthError, requires_auth, verify_decode_jwt, get_access_token, get_user_id, get_MGMT_API_ACCESS_TOKEN
...
def create_app(test_config=None):
    ...
    def login():
        ...
        elif 'delete:movie' not in permissions and 'delete:actor' in permissions:
    ...
    def not_fond(error):
        return render_template('404.html', data=session['user_info']), 404

    @app.errorhandler(500)
    def server_error(error):
        return render_template('500.html', data=session['user_info']), 500 
    ...
    def unauthorized(error):
        return  render_template('401.html', data=session['user_info']), 401
